...
def run_simulation():
    print(f"[*] Starting local credential stuffing simulation against {TARGET_URL}")
    ...
    for i, creds in enumerate(DUMMY_CREDENTIALS):
        try:
            ...
            response = requests.post(TARGET_URL, json=creds, timeout=3)
            ...
            print(f"[Attempt {i+1}/{len(DUMMY_CREDENTIALS)}] POST /login ({creds['user']}:***) - Status: {status}")
    ...
    print("\n[*] Simulation complete.")
    print(f"[*] Summary: {fail_hits} Failed Logins, {success_hits} Success Logins.")
